...
def generate_graph(graph, schema, seed, size) :
	...
	frontier = [((seed,x[1]),(x[0],x[2])) for x in graph[seed] if x[0] in schema]
...
dims = ["date_dim" + str(x) for x in range(start, end+1, m)]

joins = 2

generate_microbenchmark(dims, joins, "/joins-" + str(joins) + "-" + str(size))
"""
