...
def init_graph(schema) : 
	...
	while low :
		nexttwo = random.sample(low,2)
		graph["store_sales"] += [(nexttwo[0], "ss_sold_date_sk", "d_date_sk",)]
		graph[nexttwo[0]] = [(nexttwo[1], "d_date_sk", "d_date_sk",)]
		graph[nexttwo[1]] = [] 
		low = [x for x in low if x not in nexttwo]

	while high :
		nexttwo = random.sample(high,2)
		graph["store_sales"] += [(nexttwo[0], "ss_sold_date_sk", "d_date_sk",)]
		graph[nexttwo[0]] = [(nexttwo[1], "d_date_sk", "d_date_sk",)]
		graph[nexttwo[1]] = [] 
		high = [x for x in high if x not in nexttwo]
