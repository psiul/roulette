...
where ss.ss_customer_sk = c.c_customer_sk and ss.ss_item_sk = i.i_item_sk and c.c_current_hdemo_sk = hd.hd_demo_sk and c.c_current_cdemo_sk = cd.cd_demo_sk and c.c_current_addr_sk = a.ca_address_sk and ss.ss_sold_date_sk = d.d_date_sk and i.i_hash >= 574 and i.i_hash <= 907 and cd.cd_hash >= 2 and cd.cd_hash <= 752 and d.d_hash >= 55 and d.d_hash <= 455
;
