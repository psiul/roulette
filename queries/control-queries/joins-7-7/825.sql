...
where ss.ss_sold_date_sk = d.d_date_sk and ss.ss_customer_sk = c.c_customer_sk and c.c_current_hdemo_sk = hd.hd_demo_sk and c.c_current_cdemo_sk = cd.cd_demo_sk and c.c_current_addr_sk = a.ca_address_sk and ss.ss_item_sk = i.i_item_sk and ss.ss_hash >= 207 and ss.ss_hash <= 957 and hd.hd_hash >= 149 and hd.hd_hash <= 549 and cd.cd_hash >= 503 and cd.cd_hash <= 836
;
