...
where ss.ss_customer_sk = c.c_customer_sk and ss.ss_cdemo_sk = cd.cd_demo_sk and c.c_current_hdemo_sk = hd.hd_demo_sk and c.c_current_addr_sk = a.ca_address_sk and ss.ss_item_sk = i.i_item_sk and ss.ss_sold_date_sk = d.d_date_sk and ss.ss_hash >= 53 and ss.ss_hash <= 803 and cd.cd_hash >= 394 and cd.cd_hash <= 727 and hd.hd_hash >= 385 and hd.hd_hash <= 785
;
